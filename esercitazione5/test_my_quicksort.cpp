...
{		
	...
	for (int i=0; i<vi_dim.size();i++) {
		...
		for (int j=0; j<100; j++) {
			...
			my_quicksort(vi_test,0,vi_test.size()-1);
    ...
    my_quicksort(vi_string,0,vi_string.size()-1);
    ...
    if (sorted && string_sorted ) {
		std::cout << "L'algoritmo my_quicksort ha funzionato" << "\n";
		return EXIT_SUCCESS;
	}
    else {
		std::cout << "L'algoritmo my_quicksort non ha funzionato \n";
		return EXIT_FAILURE;
	}
}