...
{		
	...
	for (int i=0; i<vi_dim.size();i++) {
		...
		for (int j=0; j<100; j++) {
			...
			mergesort(vi_test,0,vi_test.size()-1);
    ...
    mergesort(vi_string,0,vi_string.size()-1);
    ...
    if (sorted && string_sorted ) {
		std::cout << "L'algoritmo mergesort ha funzionato" << "\n";
		return EXIT_SUCCESS;
	}
    else {
		std::cout << "L'algoritmo mergesort non ha funzionato \n";
		return EXIT_FAILURE;
	}
}